...
//Huge, huge thanks and shoutout to Uncomfy on the VRC Shader Discord for helping me figure this out <3
        ...
        float4 ChooseProjection(float2 uv, uint selection)
        {
        ...
        float ChooseProjectionScalar(uint selection, float coneWidth)
        {
            float chooser = IF(isOSC() == 1, selection, instancedGOBOSelection());
        ...
        fixed4 ProjectionFrag(v2f i) : SV_Target
        {
            ...
            if(i.color.g > 0.5)
            {
               ...
               // float oscTiltValue = GetTiltValue(sector);
                ...
                float UVscale = 1/(0 + (distanceFromOrigin * ChooseProjectionScalar(selection, i.intensityStrobeWidth.z) + (0 * (distanceFromOrigin * distanceFromOrigin))));
                ...
                float4 tex = ChooseProjection(uvCoords, selection);
