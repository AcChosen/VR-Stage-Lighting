...
//Huge, huge thanks and shoutout to Uncomfy on the VRC Shader Discord for helping me figure this out <3
        ...
        float4 ChooseProjection(float2 uv)
        {
            //float chooser = IF(isOSC() == 1, selection, instancedGOBOSelection());
            float chooser = instancedGOBOSelection();
            float4 result = tex2D(_ProjectionMainTex, uv);
            result = IF(round(chooser) == 1.0, tex2D(_ProjectionMainTex, uv), result);
            result = IF(round(chooser) == 2.0, tex2D(_ProjectionTex2, uv), result);
            result = IF(round(chooser) == 3.0, tex2D(_ProjectionTex3, uv), result);
            result = IF(round(chooser) == 4.0, tex2D(_ProjectionTex4, uv), result);
            result = IF(round(chooser) == 5.0, tex2D(_ProjectionTex5, uv), result);
            result = IF(round(chooser) == 6.0, tex2D(_ProjectionTex6, uv), result);
            return result;
        }
        float ChooseProjectionScalar(float coneWidth)
        {
            //float chooser = IF(isOSC() == 1, selection, instancedGOBOSelection());
            float chooser = instancedGOBOSelection();
            float result = _ProjectionUVMod;
            result = IF(round(chooser) == 1.0, _ProjectionUVMod, result);
            result = IF(round(chooser) == 2.0, _ProjectionUVMod2, result);
            result = IF(round(chooser) == 3.0, _ProjectionUVMod3, result);
            result = IF(round(chooser) == 4.0, _ProjectionUVMod4, result);
            result = IF(round(chooser) == 5.0, _ProjectionUVMod5, result);
            result = IF(round(chooser) == 6.0, _ProjectionUVMod6, result);
            return result * (clamp(coneWidth, -2.0, 4) + 1.6);
        } 
        ...
        fixed4 ProjectionFrag(v2f i) : SV_Target
        {
            ...
            if(i.color.g > 0.5)
            {
               ...
               // float oscTiltValue = GetTiltValue(sector);
                ...
                float UVscale = 1/(0 + (distanceFromOrigin * ChooseProjectionScalar(coneWidth) + (0 * (distanceFromOrigin * distanceFromOrigin))));
                ...
                projPos = InvertRotations(float4(projPos,1.0));
                ...
                //_SpinSpeed = IF(isOSC() == 1, _SpinSpeed * coneWidth, _SpinSpeed);
                ...
                float4 tex = ChooseProjection(uvCoords);
