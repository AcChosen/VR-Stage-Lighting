Shader "VRSL/Flasher Static/Fixture"
{
    Properties
    {
        //[Header (INSTANCED PROPERITES)]
         ...
         _Channel ("Which channel in the sector to use? (0-12)", Int) = 0
    ...
    SubShader
    {
      ...
      //  sampler2D _EmissionMask, _AlternateEmissionMask;
        ...
        // float GetTextureSampleScrollValue(uint sector)
        // {
        //     return getValueAtCoords(0.960, standardSampleYAxis, sector, _OSCGridRenderTextureRAW); 
        // }

         float4 getChannel()
        {
            return UNITY_ACCESS_INSTANCED_PROP(Props,_Channel);
        }
            float GetChannelIntensity(uint sector, uint channel)
            {
                float value = getValueAtCoords(0.038017 + ((channel-1) * 0.077166),standardSampleYAxis,sector, _OSCGridRenderTextureRAW);
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float4 col = IF(isOSC() == 1, GetChannelIntensity(sector, channel) * getEmissionColor(), getEmissionColor());
            fixed4 c = tex2D (_MainTex, IN.uv_MainTex) * _Color;
            half4 e = col;
            o.Albedo = c.rgb;
            o.Normal = UnpackNormal (tex2D (_NormalMap, IN.uv_NormalMap));
            //e = IF(isOSC() == 1,lerp(half4(-_CurveMod,-_CurveMod,-_CurveMod,1), e, pow(GetOSCIntensity(sector, 1.0), 1.0)), e);
