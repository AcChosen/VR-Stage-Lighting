...
//Huge, huge thanks and shoutout to Uncomfy on the VRC Shader Discord for helping me figure this out <3
        ...
        float ChooseProjectionScalar(float coneWidth, float projChooser)
        {
            ...
            #endif
            //float a = pow(clamp(coneWidth,1.4,1.8), 1.7);
            #ifdef WASH
                //a = 2.5;
            #endif
        ...
        fixed4 ProjectionFrag(v2f i) : SV_Target
        {
            ...
            if(i.color.g > 0.5)
            {
               ...
               // float oscTiltValue = GetTiltValue(sector);
                ...
                float3 objectOrigin = mul(unity_ObjectToWorld, float4(0.0,-0.1,0.0,1.0) ).xyz;
              ...
              //  float3 projectionOriginWorld = mul(unity_ObjectToWorld,_ProjectionRangeOrigin);
                ...
                distanceFromOrigin = lerp(distanceFromOrigin*0.6 +0.65,distanceFromOrigin, saturate(UNITY_ACCESS_INSTANCED_PROP(Props,_ConeWidth)));
                float UVscale = 1/(0 + ((distanceFromOrigin) * ChooseProjectionScalar(coneWidth, projChooser)));
                ...
                #ifdef RAW
                    float spinSpeed = IF(checkPanInvertY() == 1, -getGoboSpinSpeed(), getGoboSpinSpeed());
                    uvCoords = IF(isGOBOSpin() == 1 && projChooser > 1.0, RotateUV(uvCoords, _Time.w * ( 10* spinSpeed)), RotateUV(uvCoords, _ProjectionRotation)); 
                #else
